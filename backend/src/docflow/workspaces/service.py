...
_SELECT_WS = """
SELECT workspace_technical_key, slug, label, description, created_at, updated_at
...
"""
_UPDATE_WS = (
    "UPDATE workspace SET {cols}, updated_at = now() WHERE workspace_technical_key = $1 "
    "RETURNING workspace_technical_key, slug, label, description, created_at, updated_at"
)
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            try:
                ...
                raise HTTPException(
                    status_code=409,
                    detail=f"workspace '{data.slug}' existe déjà",
                ) from exc
...
async def update_workspace(
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            ...
            row = await conn.fetchrow(
                _UPDATE_WS.format(cols=cols), wk, *list(updates.values())
            )
    assert row is not None
    return _row(row)


async def delete_workspace(pool: asyncpg.Pool, ws_slug: str) -> None:
    async with pool.acquire() as conn:
        async with conn.transaction():
            wk = await require_workspace(conn, ws_slug)
            await conn.execute(
