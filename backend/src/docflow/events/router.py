...
code, cf. events/catalog.py).

**Endpoints PUBLICS** (pas d'authentification) : ils n'exposent que des
métadonnées de contrat (eventCodes + formes JSON Schema), aucune donnée ni
secret. C'est un endpoint de découverte destiné à être consommé par le pair
(workflow) sans lui imposer un credential docflow. Le reste de l'API reste
fail-closed.
"""
...
async def get_schema_versions(event_code: str) -> dict[str, Any]:
...
async def get_schema_version(event_code: str, version: int) -> dict[str, Any]:
