...
async def test_create_block_child(db_pool: asyncpg.Pool, test_workspace: dict) -> None:
    ...
    child = await block_svc.create_block(
        ...
        _WS,
        DataBlockCreate(
            slug="feat-1", label="Feat 1", functional_type_slug="feature", parent_slug="epic-1"
        ),
    )
    assert child.parent_slug == "epic-1"


async def test_mirror_constraint_i5_root_type(db_pool: asyncpg.Pool, test_workspace: dict) -> None:
...
async def test_mirror_constraint_i5_wrong_parent_type(
    ...
    with pytest.raises(HTTPException) as exc:
        ...
        await block_svc.create_block(
            ...
            _WS,
            DataBlockCreate(
                slug="task-1", label="Task 1", functional_type_slug="task", parent_slug="epic-1"
            ),
        )
    assert exc.value.status_code == 422
    assert "I-5" in exc.value.detail
...
async def test_block_slug_unique_per_workspace(db_pool: asyncpg.Pool, test_workspace: dict) -> None:
    await _make_types(db_pool)
    await block_svc.create_block(
...
async def test_delete_block_with_children_needs_confirm(
    ...
    await block_svc.create_block(
        ...
        _WS,
        DataBlockCreate(
            slug="f-child", label="Child", functional_type_slug="feature", parent_slug="e-root"
        ),
    )
...
async def test_dod2_create_block_root_epic_ok(db_pool: asyncpg.Pool, test_workspace: dict) -> None:
...
async def test_dod4_create_block_type_other_workspace_rejected(
    ...
    other_ws = await ws_svc.create_workspace(
        db_pool, WorkspaceCreate(slug="other-ws-dod4", label="Other DoD4"), None
    )
    try:
        ...
        with pytest.raises(HTTPException) as exc:
            await block_svc.create_block(
                db_pool,
                _WS,
                DataBlockCreate(slug="bad-block", label="Bad", functional_type_slug="solo-root"),
            )
        assert exc.value.status_code == 422
    finally:
        await db_pool.execute(
