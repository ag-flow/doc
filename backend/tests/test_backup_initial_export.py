...
async def test_first_run_exports_documents_absent_from_changelog(
    ...
    await doc_svc.create_document(
        db_pool,
        _WS,
        DocumentCreate(title="Ancien", slug="ancien", block_id=block_id, functional_type_slug="task"),
    )
    # Simule un document antérieur au journal : purge des entrées du workspace
    await db_pool.execute(
        "DELETE FROM document_change_log WHERE workspace_technical_key = $1", wk
    )
    ...
    assert result["files_written"] == 2  # ancien.md + ancien.json
    assert result["commit_sha"] is not None

    check = Repo.clone_from(str(remote_dir), tmp_path / "check", branch="main")
    tree = pathlib.Path(check.working_dir)
    assert (tree / _WS / "ancien.md").exists()
    assert (tree / _WS / "ancien.json").exists()
