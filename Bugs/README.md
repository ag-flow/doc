...
Chaque bug a **sa propre fiche** dans ce répertoire. Les fiches documentent le défaut, sa reproduction et une piste de correction.
...
| [DB-11](fixed/DB-11-url-ssh-git-malformee.md) | ✅ URL SSH git malformée (`git@host/repo` au lieu de `:`) | `backup/worker.py` | **Sonnet** |
| [DEP-01](fixed/DEP-01-identifiants-admin-bootstrap-morts.md) | ✅ Identifiants admin bootstrap morts affichés par le déploiement | `scripts/dev-deploy.sh`, `deploy/DEPLOY.md` | **Sonnet** |
| [DEP-02](fixed/DEP-02-scripts-sql-doublons-divergents.md) | ✅ `scripts/*.sql` : copies périmées et divergentes des migrations | `scripts/*.sql` | **Sonnet** |
...
| [DB-15](fixed/DB-15-nom-variable-encryption-key.md) | ✅ Messages d'erreur citant `DOCFLOW_ENCRYPTION_KEY` au lieu de `ENCRYPTION_KEY` | `vault/router.py`, `webhooks/service.py` | **Sonnet** |
| [DEP-06](fixed/DEP-06-dev-env-sans-encryption-key.md) | ✅ `/data/.env` de dev sans `ENCRYPTION_KEY` (divergence dev/prod) | `scripts/dev-deploy.sh` | **Sonnet** |
| [DEP-07](fixed/DEP-07-pg-dump-version-client-flottante.md) | ✅ Version de `pg_dump` non maîtrisée dans l'image (base flottante) | `deploy/Dockerfile` | **Sonnet** |
| [DEP-08](fixed/DEP-08-prod-deploy-source-env-docker.md) | ✅ `prod-deploy.sh` source le `.env` format docker dans bash | `deploy/prod-deploy.sh` | **Sonnet** |
| [DEP-09](fixed/DEP-09-port-dev-8080-vs-8000.md) | ✅ Port de dev : la doc dit `:8080`, uvicorn écoute sur 8000 | `CLAUDE.md`, `vite.config.ts` | **Sonnet** |
| [FE-10](fixed/FE-10-double-submit-entree-creation.md) | ✅ Double-submit par Entrée dans les dialogues de création | `AddDocumentDialog.tsx`, `DocumentChildrenPanel.tsx` | **Sonnet** |
| [FE-11](fixed/FE-11-linksearchpopup-spinner-bloque.md) | ✅ LinkSearchPopup : spinner bloqué et réponses hors-ordre | `LinkSearchPopup.tsx` | **Sonnet** |
| [FE-12](fixed/FE-12-labeltoslug-supprime-chiffres.md) | ✅ `labelToSlug` supprime les chiffres | `lib/slug.ts` | **Sonnet** |
| [FE-13](fixed/FE-13-i18n-hint-modele-contenu-vide.md) | ✅ i18n : hint du modèle de contenu vidé par l'interpolation | `TypePropertiesPanel.tsx` | **Sonnet** |
| [FE-14](fixed/FE-14-typesadmin-fragments-sans-key.md) | ✅ TypesAdmin : fragments sans `key` | `TypesAdmin.tsx` | **Sonnet** |
| [FE-15](fixed/FE-15-blocsadmin-export-sans-gestion-erreur.md) | ✅ BlocsAdmin `handleExport` : erreurs HTTP non gérées | `BlocsAdmin.tsx` | **Sonnet** |
| [FE-16](fixed/FE-16-pub-non-proxifie-dev.md) | ✅ `/pub` non proxifié en dev | `vite.config.ts`, `lib/api.ts` | **Sonnet** |
| [FE-17](fixed/FE-17-logout-cache-non-purge.md) | ✅ Logout : cache TanStack Query non purgé | `Sidebar.tsx` | **Sonnet** |
| [FE-18](fixed/FE-18-apikeys-scopes-perimes.md) | ✅ ApiKeysPage : scopes affichés périmés après sauvegarde | `ApiKeysPage.tsx` | **Sonnet** |
